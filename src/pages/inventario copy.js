...
const eliminarCliente = async (id) => {
  if (window.confirm("¿Está seguro de que desea eliminar este articulo?")) {
    try {
      await axios.delete(
...
const editarCliente = async (id, nombre, cantidad, precio, categoria) => {
  if (!nombre || !cantidad || !precio || !categoria) {
    alert("Todos los campos son obligatorios");
    return;
  }
...
function Inventario() {
  ...
  function editarClick(id) {
    ...
    setCategoria(itemArticulo[clienteIndex].categoria);
